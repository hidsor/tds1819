...
public class TestUsuario {
	...
	public void testCreacion() {
		// Comprobamos que el usuario se ha creado correctamente (con los parámetros
		// especificados y sin ser premium)
	...
	public void testPremium() {
		assertEquals("Filtro antes de pasar a premium", NoFiltro.class.getName(),
				usuario.getFiltro().getClass().getName());
		...
		assertEquals("Filtro tras salir de premium", NoFiltro.class.getName(),
				usuario.getFiltro().getClass().getName());
	...
	public void testRecientes() {
		...
		// Comprobamos si al reproducir mas de 5 videos, solo se almacenen 5 en
		// recientes
		for (int i = 2; i < 7; i++) {
	...
	public void testCrearListasVideos() {
		...
		usuario.addListaVideos(new ListaVideos("Mi lista"));
		assertEquals("Añadir dos listas con el mismo nombre", false,
				usuario.addListaVideos(new ListaVideos("Mi lista")));

		assertEquals("Añadir una lista llamada Recientes", false,
				usuario.addListaVideos(new ListaVideos(usuario.getListaRecientes().getNombre())));
